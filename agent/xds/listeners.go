...
func makeHTTPFilter(opts listenerFilterOpts) (*envoy_listener_v3.Filter, error) {
	...
	if len(opts.wasmFilters) > 0 {
		for _, f := range opts.wasmFilters {
			...
			wasm_plugin_config := &envoy_wasm_v3.PluginConfig{
				Name: f.Name,
				VmConfig: &envoy_wasm_v3.PluginConfig_InlineVmConfig{
					InlineVmConfig: &envoy_wasm_v3.VmConfig{
						VmId:             f.Name,
			...
			// Do a little dance to create the WASM filter config
			m := jsonpb.Marshaler{OrigName: true}
			buf := &bytes.Buffer{}
			err := m.Marshal(buf, wasm_plugin_config)
			if err != nil {
				return nil, err
			}

			s := &structpb.Struct{}
			err = jsonpb.Unmarshal(buf, s)
			if err != nil {
				return nil, err
			}
			...
			config := &structpb.Struct{
				Fields: map[string]*structpb.Value{
					"@type": {
						Kind: &structpb.Value_StringValue{StringValue: "type.googleapis.com/envoy.extensions.filters.http.wasm.v3.Wasm"},
					},
					"config": {
			...
			envoy_wasm_filter_config_any, err := ptypes.MarshalAny(envoy_wasm_filter_config)
			if err != nil {
				return nil, err
			}
			envoy_wasm_filter := &envoy_http_v3.HttpFilter{
				Name:       "envoy.filters.http.wasm",
				ConfigType: &envoy_http_v3.HttpFilter_TypedConfig{TypedConfig: envoy_wasm_filter_config_any},
			}
	...
	cfg.HttpFilters = append(cfg.HttpFilters, &envoy_http_v3.HttpFilter{Name: "envoy.router"})
